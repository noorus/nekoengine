...
namespace v8pp {
  ...
  namespace detail {
    ...
    template <typename T>
    using is_pointer_cast_allowed = std::integral_constant<bool, sizeof( T ) <= sizeof( void* ) && std::is_trivial<T>::value>;
    ...
    public:
      ...
      explicit pointer_cast( void* ptr )
        : ptr( ptr )
      {
      }
      explicit pointer_cast( T value )
        : value( value )
      {
      }
    ...
    void forward_ret( FunctionCallbackInfo<Value> const& args, std::false_type /*is_void_return*/ )
    {
      ...
      args.GetReturnValue().Set( converter::to_v8( args.GetIsolate(), invoke<Traits, F>( args, std::is_member_function_pointer<F>() ) ) );
    }
