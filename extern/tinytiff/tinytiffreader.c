...
void TinyTIFFReader_fopen(TinyTIFFReaderFile* tiff, const wchar_t* filename) {
  if ( tiff->customcallbacks )
  {
    tiff->customcallbacks->cbopen( tiff, filename );
    return;
  }
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
...
int TinyTIFFReader_fclose(TinyTIFFReaderFile* tiff) {
  if ( tiff->customcallbacks )
    return tiff->customcallbacks->cbclose( tiff );
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
...
{
  if ( tiff->customcallbacks )
    return tiff->customcallbacks->cbseekset( tiff, offset );
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
...
{
  if ( tiff->customcallbacks )
    return tiff->customcallbacks->cbseekcur( tiff, offset );
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
   DWORD res = SetFilePointer (tiff->hFile,
                                offset,
                                NULL,
...
{
  if ( tiff->customcallbacks )
    return tiff->customcallbacks->cbread( ptr, ptrsize, size, count, tiff );
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
    DWORD  dwBytesRead = 0;
...
{
  if ( tiff->customcallbacks )
    return tiff->customcallbacks->cbtell( tiff );
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
...
{
  if ( tiff->customcallbacks )
    return tiff->customcallbacks->cbgetpos( tiff, pos );
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
...
{
  if ( tiff->customcallbacks )
    return tiff->customcallbacks->cbsetpos( tiff, pos );
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
...
TinyTIFFReaderFile* TinyTIFFReader_open(const wchar_t* filename, TinyTiffCustomCallbacks* callbacks) {
    TinyTIFFReaderFile* tiff=(TinyTIFFReaderFile*)malloc(sizeof(TinyTIFFReaderFile));
    tiff->customcallbacks = callbacks;
    if ( callbacks )
    {
      tiff->filesize = callbacks->cbstat( tiff, filename );
    }
    else
    {
      tiff->filesize = 0;
      struct _stat64i32 file;
      if ( _wstat( filename, &file ) == 0 )
      {
