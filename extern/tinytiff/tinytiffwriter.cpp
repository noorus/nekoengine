...
/*! \brief wrapper around fopen
    ...
    \internal
 */
static void TinyTIFFWriter_fopen(TinyTIFFWriterFile* tiff, const wchar_t* filename) {
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
...
TinyTIFFWriterFile* TinyTIFFWriter_open(const wchar_t* filename, uint16_t bitsPerSample, enum TinyTIFFWriterSampleFormat sampleFormat, uint16_t samples, uint32_t width, uint32_t height, enum TinyTIFFWriterSampleInterpretation sampleInterpretation) {
