...
void main()
{
  ...
  vec2 e = texture2D(edges_tex, in_texcoord).rg;
  ...
  if (e.g > 0.0)
  {
    ...
    weights.rg = SMAACalculateDiagWeights(edges_tex, area_tex, in_texcoord, e, subsampleIndices);
    ...
    vec3 coords;
    coords.x = SMAASearchXLeft(edges_tex, search_tex, in_offset[0].xy, in_offset[2].x);
    coords.y = in_offset[1].y; // vOffset[1].y = vTexCoord0.y - 0.25 * SMAA_RT_METRICS.y (@CROSSING_OFFSET)
    d.x = coords.x;

    // Now fetch the left crossing edges, two at a time using bilinear
    // filtering. Sampling at -0.25 (see @CROSSING_OFFSET) enables to
    // discern what value each edge has:
    float e1 = texture2D(edges_tex, coords.xy).r; // LinearSampler

    // Find the distance to the right:
    coords.z = SMAASearchXRight(edges_tex, search_tex, in_offset[0].zw, in_offset[2].y);
    d.y = coords.z;

    // We want the distances to be in pixel units (doing this here allow to
    // better interleave arithmetic and memory accesses):
    d = abs(round(mad(SMAA_RT_METRICS.zz, d, -in_pixcoord.xx)));
    ...
    float e2 = SMAASampleLevelZeroOffset(edges_tex, coords.zy, vec2(1, 0)).r;
  ...
  if (e.r > 0.0)
  {
    ...
    vec3 coords;
    coords.y = SMAASearchYUp(edges_tex, search_tex, in_offset[1].xy, in_offset[2].z);
    coords.x = in_offset[0].x; // vOffset[1].x = vTexCoord0.x - 0.25 * SMAA_RT_METRICS.x;
    d.x = coords.y;

    // Fetch the top crossing edges:
    float e1 = texture2D(edges_tex, coords.xy).g; // LinearSampler

    // Find the distance to the bottom:
    coords.z = SMAASearchYDown(edges_tex, search_tex, in_offset[1].zw, in_offset[2].w);
    d.y = coords.z;

    // We want the distances to be in pixel units:
    d = abs(round(mad(SMAA_RT_METRICS.ww, d, -in_pixcoord.yy)));
    ...
    float e2 = SMAASampleLevelZeroOffset(edges_tex, coords.xz, vec2(0, 1)).g;

    // Get the area for this direction:
    weights.ba = SMAAArea(area_tex, sqrt_d, e1, e2, subsampleIndices.x);

    // Fix corners:
    coords.x = in_texcoord.x;
