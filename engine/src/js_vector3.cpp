...
namespace neko {
  ...
  namespace js {
      ...
      else if ( args.Length() == 1 )
      {
        ...
        else if ( args[0]->IsArray() )
        {
          ...
          if ( arr->Length() == 3 )
          {
            ...
            vec.z = static_cast<Real>( arr->Get( context, 2 ).ToLocalChecked()->NumberValue( context ).FromMaybe( 0.0 ) );
            valueParsed = true;
          }
        }
    ...
    //! vec3 vec3.add( vec3 )
    //! \endverbatim
    void Vector3::js_add( const V8CallbackArgs& args )
    {
      auto isolate = args.GetIsolate();
      HandleScope handleScope( isolate );
    ...
    //! vec3 vec3.sub( vec3 )
    //! \endverbatim
    void Vector3::js_sub( const V8CallbackArgs& args )
    {
      auto isolate = args.GetIsolate();
      HandleScope handleScope( isolate );
