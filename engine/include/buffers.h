...
namespace neko {
  ...
  public:
    ...
    inline void setFrom( const vector<VertexType>& verts, const vector<GLuint>& idxs )
    {
      setFrom( verts.size(), verts.data(), idxs.size(), idxs.data() );
    }
    inline void setFrom( const pair<vector<VertexType>, vector<GLuint>>& source )
    {
  ...
  using Indexed3DVertexBuffer = IndexedVertexBufferBase<Vertex3D>;
